...
for (const [base, cns] of byBase) {
  ...
  const baseInfo = {
    base, dirCount: cns.length,
    localTermini: [...new Set(localTermini.map((t) => t[0] && t[1] ? `${t[0]} ⇄ ${t[1]}` : t[0] || t[1] || ''))].join(' / '),
    firstTime, lastTime, company: meta?.company_cn || '',
  };
  if (!raw || raw.status !== 'found' || !raw.foundLines?.length) {
    rows.push({ ...baseInfo, status: '车来了未收录（可能停运/改名/未收录）', chTermini: '' });
    continue;
  }
  ...
  if (matchedDirs.length === 0) {
    rows.push({ ...baseInfo, status: '同名但站点差异大（疑似线路调整）', chTermini });
    adjusted.push({ ...baseInfo, status: '疑似线路调整', chTermini });
    continue;
  }
  rows.push({ ...baseInfo, status: '可核对（方向记录已匹配）', chTermini });
}
...
rows.sort((a, b) => (order[a.status] - order[b.status]) || a.base.localeCompare(b.base, 'zh-CN'));
writeCsv(`${OUT_DIR}/线路状态核对_车来了.csv`, header, rows);
writeCsv(`${OUT_DIR}/线路状态核对_疑似调整.csv`, header, adjusted);

const count = rows.reduce((o, r) => { o[r.status] = (o[r.status] || 0) + 1; return o; }, {});
console.log(JSON.stringify(count, null, 2));
console.log('疑似调整线路:');
adjusted.slice(0, 30).forEach((r) => console.log('  -', r.base, '|', r.chTermini));
